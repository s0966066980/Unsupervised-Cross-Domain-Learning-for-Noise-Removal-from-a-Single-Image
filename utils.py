...
def azimuthalAverage(image, center=None):
    ...
    return radial_prof[1:-2]  # 我們忽略最後兩個極高頻段以避免噪聲。

def azimuthalAverageF(image, center=None):
    """
    計算方位角平均徑向剖面。
    圖像 - 2D 圖像
    center - 用作中心的 [x,y] 像素坐標。默認是
    ...
    tbin = np.bincount(radius, image.ravel())
    radial_prof = tbin / (nr + 1e-10)
    return radial_prof[1:]  # 我們忽略最後兩個極高頻段以避免噪聲。    
...
def get_fft_featureF(x):
    ...
    out = []
    for i in range(magnitude_spectrum.shape[0]):
        out.append(torch.from_numpy(azimuthalAverageF(magnitude_spectrum[i])).float().unsqueeze(0))
    out = torch.cat(out, dim=0)
