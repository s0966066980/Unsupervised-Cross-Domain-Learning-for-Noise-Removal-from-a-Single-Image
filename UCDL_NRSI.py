...
class UCDL_NRSI(object):
    ...
    def build_model(self, args):
        ...
        self.D_optim = optim.Adam(
            itertools.chain(
                self.disC.parameters(), self.disN.parameters(), self.disT.parameters(), self.disS.parameters()
            ),
            lr=args.lrD,
            betas=(0.5, 0.999),
            weight_decay=0.0001,
    ...
    def train(self, args):
        ...
        self.disC.train().to(self.device), self.disN.train().to(self.device), self.disT.train().to(self.device), self.disS.train().to(self.device)
        ...
        for epoch in range(last_epoch, args.n_epochs):
            ...
            self.disC.train(), self.disN.train(),self.disT.train(), self.disS.train()
            ...
            for i, (Noisy, Clean) in enumerate(self.train_loader):
                ...
                G_ad_loss = args.adv_w * G_ad_loss_img  + args.adv_w * G_ad_loss_img_clean  + args.texture_w * G_ad_loss_texture  + args.fft_w * G_ad_loss_spectral 
